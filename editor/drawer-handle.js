// Guaranteed escape hatch for every editor sheet. This does not own layout/content;
// it only enforces the UX invariant that an opened sheet always has a visible way out.

const body = document.body;
const iframe = document.getElementById('engine-frame');

function closeOuterSheets() {
  body.classList.remove('inspector-open', 'story-open');
...
addHandle(document.getElementById('timeline-dock'), 'Story schließen');
...
// Spatial PIPELINE/WELT sheets live inside the same-origin engine iframe. Give those
// exactly the same explicit downward escape marker instead of relying on a toolbar
// toggle or a tap on the 3D canvas.
function installSpatialHandles() {
  ...
  if (!doc.getElementById('spatial-sheet-handle-style')) {
    ...
    style.textContent = `
      ...
      .spatial-sheet-drop{
        position:sticky;top:0;left:50%;z-index:30;display:block;
        width:88px;height:34px;min-height:34px;margin:-5px auto 7px;
        padding:0 0 7px;border:1px solid rgba(255,255,255,.22);border-top:0;
        border-radius:0 0 18px 18px;background:rgba(18,19,23,.98);color:#e5e7eb;
        font:800 24px/1 system-ui,sans-serif;touch-action:manipulation
      }
    `;
