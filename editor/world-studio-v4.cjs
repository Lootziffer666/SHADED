...
const BRIDGE_URL = 'http://127.0.0.1:49666';

const SKY_PRESETS = {
  day: { label: 'Klarer Tag', params: { dayNight: .03, storm: .02, fog: .03, glow: .08, wind: .22 } },
  golden: { label: 'Golden Hour', params: { dayNight: .18, storm: .04, fog: .08, glow: .22, wind: .18 } },
  overcast: { label: 'Bedeckt', params: { dayNight: .12, storm: .45, fog: .18, glow: .18, wind: .35 } },
  night: { label: 'Nacht', params: { dayNight: .92, storm: .08, fog: .12, glow: .62, wind: .16 } },
  storm: { label: 'Sturm', params: { dayNight: .52, storm: .95, rain: .72, wet: .88, puddle: .62, fog: .42, glow: .48, wind: .95 } },
};

const WORLD_PRESETS = {
  compact: { label: 'Kompakt', boundary: .18, radius: 7, thickness: .025, textureBlend: .82, maxEdge: 768, pointBudget: 300000 },
  room: { label: 'Raum', boundary: .35, radius: 12, thickness: .035, textureBlend: .75, maxEdge: 1024, pointBudget: 500000 },
  wide: { label: 'Weit', boundary: .62, radius: 22, thickness: .05, textureBlend: .68, maxEdge: 1280, pointBudget: 850000 },
};

const MATERIAL_PRESETS = {
  liquid: { label: 'Flüssigkeit', roughness: .08, height: .14, normal: .18, reflectivity: .92, refraction: .72, flow: .82, params: { wet: 1, puddle: .88, rain: .05, fog: .03 } },
  wetstone: { label: 'Nasser Stein', roughness: .34, height: .42, normal: .58, reflectivity: .58, refraction: .05, flow: .1, params: { wet: .82, puddle: .24, decay: .12 } },
  metal: { label: 'Metall', roughness: .22, height: .2, normal: .34, reflectivity: .88, refraction: 0, flow: 0, params: { wet: .16, bleach: .3 } },
  glass: { label: 'Glas', roughness: .04, height: .08, normal: .12, reflectivity: .78, refraction: .92, flow: 0, params: { wet: .24, glow: .38 } },
  snow: { label: 'Schnee', roughness: .76, height: .74, normal: .46, reflectivity: .28, refraction: .06, flow: 0, params: { snow: .92, snowfall: .16, wet: .28, temperature: .12 } },
  organic: { label: 'Organisch', roughness: .64, height: .56, normal: .66, reflectivity: .18, refraction: 0, flow: .08, params: { bloom: .6, wet: .32, decay: .1 } },
  emissive: { label: 'Emissiv', roughness: .2, height: .18, normal: .22, reflectivity: .5, refraction: .1, flow: 0, params: { glow: 1 } },
  neutral: { label: 'Neutral', roughness: .48, height: .35, normal: .4, reflectivity: .3, refraction: 0, flow: 0, params: {} },
};
...
function waitReady(timeout = 20000) {
  return new Promise((resolve, reject) => {
    const started = performance.now();
    const tick = () => {
...
function setStatus(message, tone = '') {
  const world = document.getElementById('world-status');
  if (world) { world.textContent = message; world.className = `world-status ${tone}`.trim(); }
...
function createStudio() {
  document.getElementById('world-studio')?.remove();
  if (!viewport) return;
  ...
  el.innerHTML = `
    ...
    <div class="world-studio-body">
      <div class="world-studio-step">
        ...
        <div class="world-import-note" id="world-import-note">Bestehende Provider-Ausgaben können ohne Neuberechnung geladen werden.</div>
      </div>
      <div class="world-studio-step">
        ...
        <div class="world-chip-grid" id="world-provider-grid">
          ...
          <button class="world-chip" data-provider="software"><b>Software</b><span>Browser-Fallback</span></button>
        </div>
      </div>
      <div class="world-studio-step">
...
function setSceneFile(file) {
  ...
  setStatus('Bild bereit. Erzeugen startet die vollständige Pipeline.');
}

async function loadDemo() {
  setStatus('Lade Demo-Bild …');
  ...
  setStatus('Demo geladen. Erzeugen startet Depth → Welt → Ego.', 'ready');
}
...
function drawMaterialPreview() {
  ...
  const hues = { liquid:[48,150,220], snow:[220,232,242], metal:[120,132,148], glass:[90,170,195], organic:[70,128,76], emissive:[78,118,255], neutral:[118,105,94], wetstone:[92,89,84] };
  const base = hues[selectedMaterial] || hues.neutral;
  ...
  sphere.addColorStop(0, `rgb(${Math.min(255,base[0]+spec)},${Math.min(255,base[1]+spec)},${Math.min(255,base[2]+spec)})`);
  sphere.addColorStop(.22, `rgb(${base.join(',')})`);
  sphere.addColorStop(1, `rgb(${Math.round(base[0]*.18)},${Math.round(base[1]*.18)},${Math.round(base[2]*.18)})`);
  ...
  ctx.strokeStyle = '#fff'; ctx.lineWidth = 1 + material.roughness * 2;
  const lines = 12 + Math.round(material.normal * 24);
  for (let i=0;i<lines;i++) {
    const y = cy-radius + (i/Math.max(1,lines-1))*radius*2;
    ctx.beginPath();
    for (let x=cx-radius;x<=cx+radius;x+=10) {
      const wave = Math.sin(x*.045+i*.7+performance.now()*.001*material.flow)*(4+material.height*10);
...
function wireStudio(el) {
  ...
  document.getElementById('world-demo')?.addEventListener('click', () => loadDemo().catch(error => setStatus(error.message, 'error')));
...
function applyEnvironmentParams() {
  ...
  const bias = selectedSky === 'night' ? 0 : Math.max(-.12, Math.min(.24, (.52-sun)*.45));
  api.setParams?.({ ...sky.params, ...MATERIAL_PRESETS[selectedMaterial].params, dayNight: Math.max(0, Math.min(1, sky.params.dayNight + bias)) });
}
...
async function importProviderFiles(fileList) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      try { jsonFiles.push({ file, data: JSON.parse(await file.text()) }); } catch {}
    }
    const bundleEntry = jsonFiles.find(entry => entry.data?.format === 'SHADED.spatial-provider-bundle.v1');
    if (bundleEntry) return setImportedBundle(bundleEntry.data, bundleEntry.file.name);
    ...
    for (const [name, descriptor] of Object.entries(result.channels || {})) {
      ...
      channelData[name] = { encoding:'base64', bytes:bytes.byteLength, data:bytesToBase64(bytes) };
      await yieldFrame();
    }
    setImportedBundle({ format:'SHADED.spatial-provider-bundle.v1', result, channelData }, resultEntry.file.name);
  } catch (error) {
    setStatus(`Provider-Import fehlgeschlagen: ${error.message}`, 'error');
  }
}
...
function setImportedBundle(bundle, name='Provider-Bundle') {
  ...
  const title = document.getElementById('world-file-title');
  const copy = document.getElementById('world-file-copy');
  const note = document.getElementById('world-import-note');
  if (title) title.textContent = name;
  if (copy) copy.textContent = `${bundle.result.provider} · ${bundle.result.modelVersion}`;
  if (note) note.textContent = 'Provider-Ergebnis geladen – Depth wird nicht erneut berechnet.';
  setStatus(`Provider-Ergebnis bereit: ${bundle.result.provider}.`, 'ready');
}
...
async function bridgeReachable() {
  if (bridgeKnownReachable) return true;
  const sameOrigin = ['127.0.0.1','localhost'].includes(location.hostname);
  if (sameOrigin) { bridgeKnownReachable = true; return true; }
  try {
    const response = await fetchWithTimeout(`${BRIDGE_URL}/api/health`, { cache:'no-store' }, 750);
    bridgeKnownReachable = response.ok;
    return bridgeKnownReachable;
  } catch {
    return false;
  }
}
...
async function requestLocalDepth(file) {
  if (selectedProvider === 'software') return { ok:false, skipped:true, attempts:[] };
  if (!(await bridgeReachable())) {
    return { ok:false, skipped:true, attempts:[{ provider:selectedProvider, ok:false, message:'Lokale RTX-Bridge nicht erreichbar' }] };
  }
  ...
  const payload = {
    ...
    boundaryRadius:preset.radius,
    mirrorThickness:preset.thickness,
    textureBlend:preset.textureBlend,
    skyPreset:selectedSky,
    sunElevation:Number(document.getElementById('world-sun')?.value ?? .62),
    materialPreset:selectedMaterial,
  };
  const url = ['127.0.0.1','localhost'].includes(location.hostname) ? '/api/generate' : `${BRIDGE_URL}/api/generate`;
  try {
    const response = await fetch(url, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify(payload) });
    if (!response.ok) throw new Error(`Bridge HTTP ${response.status}`);
    return await response.json();
  } catch (error) {
    return { ok:false, attempts:[{ provider:selectedProvider, ok:false, message:error?.message || 'Lokale GPU-Bridge fehlgeschlagen.' }] };
  }
}
...
async function browserSoftwareBundle(file) {
  ...
  const maxEdge = mobile ? 224 : 320;
  const maxPoints = mobile ? 36000 : 70000;
  ...
  for (let y=0;y<h;y++) for (let x=0;x<w;x++) {
    const i=y*w+x, p=i*4;
    const lum=(rgba[p]*.299+rgba[p+1]*.587+rgba[p+2]*.114)/255;
    const vertical=1-y/Math.max(1,h-1);
    depth[i]=.2+.8*Math.max(0,Math.min(1,.62*vertical+.38*(1-lum)));
  }
  await yieldFrame();

  const step = Math.max(1, Math.ceil(Math.sqrt((w*h)/maxPoints)));
  const rows=Math.ceil(h/step), cols=Math.ceil(w/step);
  const points=new Float32Array(rows*cols*6); let n=0;
  for (let y=0;y<h;y+=step) for (let x=0;x<w;x+=step) {
    const i=y*w+x,p=i*4,z=.2+depth[i]*.8,o=n*6;
    points[o]=(x-w*.5)/w*z; points[o+1]=(h*.5-y)/w*z; points[o+2]=z;
    points[o+3]=rgba[p]/255; points[o+4]=rgba[p+1]/255; points[o+5]=rgba[p+2]/255; n++;
  }
  await yieldFrame();

  const pack = array => {
    const bytes = new Uint8Array(array.buffer, array.byteOffset, array.byteLength);
    return { encoding:'base64', bytes:bytes.byteLength, data:bytesToBase64(bytes) };
  };
  const result = {
    format:'SHADED.spatial-provider-result.v1', provider:'shaded-browser-software', modelVersion:'luma-perspective-v2', device:'browser-cpu', precision:'fp32',
    depthConvention:'relative-depth-higher-far', metric:false,
    channels:{ depth:{file:'depth.f32',dtype:'float32-le',shape:[h,w]}, points:{file:'points.f32',dtype:'float32-le',shape:[n,6]} },
    camera:{width:w,height:h,fx:w,fy:w,cx:w*.5,cy:h*.5},
    provenance:{class:'INFERRED',sourceSha256:'0'.repeat(64),sourceFile:file.name,sourceSize:{width:sourceWidth,height:sourceHeight},processedSize:{width:w,height:h},provider:'shaded-browser-software',modelVersion:'luma-perspective-v2',parameters:{pointBudget:maxPoints}},
  };
...
function decodeFloatChannel(bundle, name) {
  const descriptor=bundle?.result?.channels?.[name], packed=bundle?.channelData?.[name];
  if (!descriptor || !packed?.data || descriptor.dtype !== 'float32-le') return null;
  const binary=atob(packed.data), bytes=new Uint8Array(binary.length);
  for (let i=0;i<binary.length;i++) bytes[i]=binary.charCodeAt(i);
  const view=new DataView(bytes.buffer), floats=new Float32Array(bytes.length/4);
  for (let i=0;i<floats.length;i++) floats[i]=view.getFloat32(i*4,true);
  return { data:floats, shape:descriptor.shape };
}

function bundleSpatialPoints(bundle, pointBudget=70000) {
  const channel=decodeFloatChannel(bundle,'points');
  if (!channel || channel.shape?.length !== 2) return null;
  const stride=channel.shape[1], count=channel.shape[0], step=Math.max(1,Math.ceil(count/pointBudget));
  const raw=[]; let min=[Infinity,Infinity,Infinity], max=[-Infinity,-Infinity,-Infinity];
  for (let i=0;i<count;i+=step) {
    const o=i*stride,x=channel.data[o],y=channel.data[o+1],z=channel.data[o+2];
    const colorScale=stride===6&&Math.max(channel.data[o+3],channel.data[o+4],channel.data[o+5])<=1?255:1;
    const p={x,y,z,r:stride===6?channel.data[o+3]*colorScale:160,g:stride===6?channel.data[o+4]*colorScale:160,b:stride===6?channel.data[o+5]*colorScale:160,confidence:.72,material:'unknown'};
    raw.push(p);
    min[0]=Math.min(min[0],x);min[1]=Math.min(min[1],y);min[2]=Math.min(min[2],z);
    max[0]=Math.max(max[0],x);max[1]=Math.max(max[1],y);max[2]=Math.max(max[2],z);
  }
  if (!raw.length) return null;
  const center=min.map((v,a)=>(v+max[a])*.5),span=Math.max(...min.map((v,a)=>max[a]-v),1e-6),s=1.7/span;
  return raw.map((p,i)=>({...p,x:(p.x-center[0])*s,y:(p.y-center[1])*s,z:(p.z-center[2])*s,sourceIndex:i}));
}

function deriveSurfaceMaps(bundle) {
  const depth=decodeFloatChannel(bundle,'depth');
  if (!depth || depth.shape.length !== 2) return null;
  const [h,w]=depth.shape; let lo=Infinity,hi=-Infinity;
  for (const v of depth.data) if (Number.isFinite(v)) { lo=Math.min(lo,v); hi=Math.max(hi,v); }
  const range=Math.max(1e-8,hi-lo), scale=Math.min(1,256/Math.max(w,h)), cw=Math.max(1,Math.round(w*scale)), ch=Math.max(1,Math.round(h*scale));
  const heightCanvas=document.createElement('canvas'), bumpCanvas=document.createElement('canvas');
  heightCanvas.width=bumpCanvas.width=cw; heightCanvas.height=bumpCanvas.height=ch;
  const hctx=heightCanvas.getContext('2d'),bctx=bumpCanvas.getContext('2d'),himg=hctx.createImageData(cw,ch),bimg=bctx.createImageData(cw,ch);
  const sample=(x,y)=>{const sx=Math.min(w-1,Math.round(x/Math.max(1,cw-1)*(w-1))),sy=Math.min(h-1,Math.round(y/Math.max(1,ch-1)*(h-1)));return(depth.data[sy*w+sx]-lo)/range;};
  for(let y=0;y<ch;y++)for(let x=0;x<cw;x++){
    const i=(y*cw+x)*4,v=Math.max(0,Math.min(1,sample(x,y))),g=Math.round(v*255),dx=sample(Math.min(cw-1,x+1),y)-sample(Math.max(0,x-1),y),dy=sample(x,Math.min(ch-1,y+1))-sample(x,Math.max(0,y-1)),nx=Math.max(-1,Math.min(1,-dx*5*material.normal)),ny=Math.max(-1,Math.min(1,-dy*5*material.normal)),nz=1/Math.sqrt(1+nx*nx+ny*ny);
    himg.data.set([g,g,g,255],i); bimg.data.set([Math.round((nx*.5+.5)*255),Math.round((ny*.5+.5)*255),Math.round(nz*255),255],i);
  }
...
async function openSpatialWorld(bundle) {
  const api=engine(),doc=engineDoc();
  if (!api || !doc) throw new Error('Engine-Dokument fehlt.');
  if (!bundle) throw new Error('Keine räumlichen Daten vorhanden.');
  applySpatialControls();
  const imported=api.spatial?.voxel?.importProviderBundle?.(bundle);
  if (!imported) throw new Error('Provider-Bundle konnte nicht in die Raumwelt übernommen werden.');
  const spatialPoints=bundleSpatialPoints(bundle, matchMedia('(max-width:860px)').matches ? 36000 : 70000);
  const originalPointCloud=api.spatial?.pointCloud;
  ...
  try {
    ...
    if (api.spatial && originalPointCloud) api.spatial.pointCloud=originalPointCloud;
  }
  applySpatialControls();
...
async function flyIntoWorld() {
  const viewer=engine()?.spatial?.viewer; if (!viewer) return;
  viewer.setMode?.('orbit');
  const start={x:0,y:.12,z:3.25,yaw:-.32,pitch:-.08}, end={x:0,y:.08,z:1.05,yaw:0,pitch:0};
  viewer.setCamera?.(start);
  const begun=performance.now(),duration=1000;
  await new Promise(resolve=>{const tick=now=>{const t=Math.min(1,(now-begun)/duration),s=t*t*(3-2*t),frame={};for(const key of Object.keys(start))frame[key]=start[key]+(end[key]-start[key])*s;viewer.setCamera?.(frame);if(t<1)requestAnimationFrame(tick);else resolve();};requestAnimationFrame(tick);});
  viewer.setMode?.('walk'); viewer.setCamera?.({yaw:0,pitch:0});
}

async function runWorldPipeline() {
  if (running) return;
  if (!sceneFile && !importedBundle) {
    document.getElementById('world-studio')?.classList.remove('collapsed');
    setStatus('Bild laden, Demo starten oder vorhandene DA2/DA3-Dateien importieren.', 'error');
    return;
  }
  running=true; worldReady=false; generatedMaps=null;
  const generate=document.getElementById('world-generate'); if(generate)generate.disabled=true;
  buildProgress();
  let bundle=importedBundle;
  try {
    const api=await waitForEngine();
    if(sceneFile){stage('image','running');await api.loadImageFile(sceneFile,false);applyEnvironmentParams();stage('image','done',sceneFile.name);}else stage('image','done','Provider-Import');
    await yieldFrame();

    stage('depth','running',bundle?'vorhanden':selectedProvider==='software'?'Browser-Software':'Provider wählen');
    if(!bundle){
      ...
      if(providerResult?.bundle){
        ...
        const failed=(providerResult.attempts||[]).filter(item=>!item.ok).map(item=>item.provider);
        stage('depth',failed.length?'fallback':'done',failed.length?`${failed.join(' → ')} → ${providerResult.provider}`:(providerResult.provider||bundle.result?.provider||'GPU'));
      }else{
        setStatus(selectedProvider==='software'?'Software-Depth läuft …':'Keine lokale RTX-Bridge – Software-Fallback läuft …');
        bundle=await browserSoftwareBundle(sceneFile);
        const failed=(providerResult?.attempts||[]).filter(item=>!item.ok).map(item=>item.provider).join(' → ');
        stage('depth','fallback',failed?`${failed} → Browser-Software`:'Browser-Software');
      }
    } else stage('depth','done',bundle.result?.provider||'Import');

    lastBundle=bundle;
    await yieldFrame();
    stage('maps','running');
    generatedMaps=deriveSurfaceMaps(bundle);
    stage('maps','done','Depth + Point Cloud + Height/Bump/Normal');
    await yieldFrame();

    stage('world','running');
    if(sceneFile){api.erstellen();try{await waitReady();}catch{}}
    applyEnvironmentParams();
    await openSpatialWorld(bundle);
    stage('world','done','Point Cloud → Voxel → Spiegelhülle → Raum');

    stage('camera','running');
    await flyIntoWorld();
    stage('camera','done','Ego aktiv');
    worldReady=true;
    setStatus(`Welt bereit · ${bundle.result?.provider||'Provider'} · direkt steuerbar`,'ready');
    setTimeout(()=>document.getElementById('world-studio')?.classList.add('collapsed'),500);
  } catch(error) {
    ...
    running=false; if(generate)generate.disabled=false;
  }
}

async function enterRoom() {
  if (worldReady && engine()?.spatial?.viewer) {
    engineDoc()?.getElementById('spatial-viewer')?.removeAttribute('hidden');
    engine()?.spatial?.viewer?.setMode?.('walk');
    return true;
  }
  if (lastBundle && !running) {
    await openSpatialWorld(lastBundle); await flyIntoWorld(); worldReady=true; return true;
  }
  if (sceneFile || importedBundle) { await runWorldPipeline(); return worldReady; }
  document.getElementById('world-studio')?.classList.remove('collapsed');
  setStatus('Bild laden oder Demo starten. SHADED erzeugt Depth und Welt automatisch.');
  return false;
}
...
document.addEventListener('click', event => {
  if (!event.target.closest?.('#btn-erstellen')) return;
  if (!sceneFile && !importedBundle) return;
...
window.SHADEDWorldStudio = {
  ...
  loadDemo,
  importProviderFiles,
  state:()=>({selectedProvider,selectedWorld,selectedSky,selectedMaterial,material:{...material},hasImage:!!sceneFile,hasImportedBundle:!!importedBundle,worldReady,bridgeKnownReachable}),
  maps:()=>generatedMaps,
  bundle:()=>lastBundle,
