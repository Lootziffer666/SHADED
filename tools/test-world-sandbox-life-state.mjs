...
// chain actually happens as a SEQUENCE of state changes driven by two real inputs (moisture and
// light), not as an animation triggered by name: rain (water up, stress down, growth up), drought
// (water down, stress up, growth down/wilting), bloom (only once age/energy/calm all hold at
// once), death (health hits zero -> alive=false -> dead wood persists, doesn't vanish or freeze),
// and light itself (a real multiplier on energy production, not decoration -- a watered, calm
// plant left in the dark still fails to produce energy or grow).
//
// All tests below run at light=1 (full light) unless testing light specifically, so the
// moisture-driven assertions stay isolated from the light multiplier -- the same isolation
// discipline used for the wilt-gate check in the drought test below.
...
function run(life, moisture, light, seconds) {
  const steps = Math.round(seconds / DT);
  for (let i = 0; i < steps; i++) stepPlantLifeState(life, DT, moisture, light);
}
...
{
  const life = createPlantLifeState();
  run(life, 0.9, FULL_LIGHT, 20); // start healthy and watered so the drop is an observable transition
  ...
  for (let i = 0; i < Math.round(30 / DT) && crossedWiltAt === null; i++) {
    stepPlantLifeState(life, DT, 0.0, FULL_LIGHT);
...
// --- Bloom: needs age AND energy AND calm together, not any one alone ---------------------
{
  ...
  run(tooYoung, 0.9, FULL_LIGHT, 10); // well short of LIFE_BLOOM_AGE_THRESHOLD=30
  ...
  run(stressedElder, 0.0, FULL_LIGHT, 60);
  ...
  run(healthyElder, 0.9, FULL_LIGHT, 60);
  ...
  run(healthyElder, 0.0, FULL_LIGHT, 30);
  assert.ok(!healthyElder.bloom, 'a bloom closes again once the plant starts wilting under new stress');
}

// --- Light: energy production needs light too, not just water+calm -----------------------
{
  // Perfect water and perfect calm (moisture held high from birth, so stress never builds), but
  // total darkness the whole time -- if light is a real multiplier rather than decoration, this
  // plant must fail to produce meaningful energy or grow, exactly like a dry plant in full sun.
  const inTheDark = createPlantLifeState();
  run(inTheDark, 0.9, 0, 20);
  assert.ok(inTheDark.energy < 0.3, `a watered, calm plant with zero light fails to build energy (got ${inTheDark.energy.toFixed(3)}) -- light is a real multiplier, not decoration`);
  assert.equal(inTheDark.growth, 0, 'a plant with zero light and starved energy does not grow, despite perfect water and calm');

  // The same plant, same starting conditions, but with real light -- must clearly outperform its
  // dark twin, isolating light as the one variable that changed.
  const inTheLight = createPlantLifeState();
  run(inTheLight, 0.9, FULL_LIGHT, 20);
  assert.ok(inTheLight.energy > inTheDark.energy + 0.3, `an identical plant given real light produces measurably more energy than its dark twin (dark=${inTheDark.energy.toFixed(3)}, lit=${inTheLight.energy.toFixed(3)})`);
}
...
// --- Death: health hits zero, plant goes inert but the object persists (dead wood) --------
{
  ...
  run(life, 0.5, FULL_LIGHT, 20); // even with moisture nearby, a dead plant no longer tracks it
...
console.log('life-state: rain/drought move water+stress+growth as a real sequence, bloom needs age+energy+calm together and closes on new stress, light is a real multiplier on energy (not decoration), and death leaves dead wood that keeps drying instead of vanishing');
