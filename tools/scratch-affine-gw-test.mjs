// SCRATCH -- Runde 28 diagnosis of the "4 von 6 schweben" viewer bug. Copy of
// scratch-village-reconstruct-affine.mjs with GROUND_WEIGHT made an env var
// (GW=...) plus two debug prints: raw (pre-sign-fix) T[.][1]/scale[.][1] per
// house, and which houses flip on axis 1 in the E4 sign-gauge fix. Verdict:
// GROUND_WEIGHT already works exactly as intended (raw T[.][1]=0.00 for all
// 6 houses, weight x10000 changes nothing because the constraint was already
// satisfied) -- the solver is innocent. The real shared ground corner is
// T[1]+scale[1] (=0.0000 for all 6 houses), not T[1] alone; the viewer script
// used the wrong one. See docs/village-box-cultivation-experimente.md Runde 28.
//
...
function solveJointAffine() {
  ...
  const GROUND_WEIGHT = Number(process.env.GW || 50000);
  ...
  console.log('DEBUG raw (pre-signfix) T[.][1] and scale[.][1]:', cubeNames.map(n => `${n}:T1=${T[n][1].toFixed(2)},L1=${scale[n][1].toFixed(2)}`).join('  '));
  ...
  cubeNames.forEach((name) => {
    ...
    for (let axis = 0; axis < 3; axis++) {
      if (scale[name][axis] < 0) {
        if (axis === 1) console.log('DEBUG axis-1 FLIP for', name);
