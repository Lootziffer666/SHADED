...
const raw = JSON.parse(fs.readFileSync('/tmp/claude-0/-home-user-SHADED/28c78061-b0e0-5f7f-bdfd-27d37e45d96b/scratchpad/newtest-raw2d-cultivated.json', 'utf8'));
...
const state = { cubes: Object.fromEntries(cubeNames.map(n => [n, { vertices: rawCubes[n].rawPoly.map(([x, y]) => [x, y]) }])) };
...
for (const base of baseCandidates) {
  for (let k = 0; k < 4; k++) {
    ...
    const angle = (restartRand() * 50 - 25) * Math.PI / 180; // +-25deg random perturbation
...
state2.R = calibrated[0].R; state2.pp = calibrated[0].pp; state2.f = calibrated[0].f;
console.log('Phase 1 winner (cheirality-first): f=' + state2.f.toFixed(1) + ' pp=[' + state2.pp.map(v => v.toFixed(1)) + ']  cheiralityBad=' + (calibrated[0].cheiralityBad * 100).toFixed(0) + '%');
...
// refinement).
console.log('\nFinal per-cube T (camera-space) and reprojection error against measured 2D vertices:');
const finalT = {};
for (const name of cubeNames) {
  const T = solveTranslation(state.cubes[name].vertices, localCoords[name], state2.R, state2.f, state2.pp);
  finalT[name] = T;
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const cam = [0, 1, 2].map(d => T[d] + a * state2.R[0][d] + b * state2.R[1][d] + c * state2.R[2][d]);
    const proj = [state2.pp[0] + state2.f * cam[0] / cam[2], state2.pp[1] + state2.f * cam[1] / cam[2]];
    const err = Math.hypot(proj[0] - state.cubes[name].vertices[i][0], proj[1] - state.cubes[name].vertices[i][1]);
    sumErr += err; maxErr = Math.max(maxErr, err);
  }
  console.log(`  ${name}: T=[${T.map(v => v.toFixed(2))}]  avgReprojError=${(sumErr / 6).toFixed(2)}px  maxReprojError=${maxErr.toFixed(2)}px`);
}
fs.writeFileSync('/tmp/claude-0/-home-user-SHADED/28c78061-b0e0-5f7f-bdfd-27d37e45d96b/scratchpad/newtest-reconstructed-cultivated.json', JSON.stringify({ W, H, R: state2.R, pp: state2.pp, f: state2.f, T: finalT, vertices: Object.fromEntries(cubeNames.map(n => [n, state.cubes[n].vertices])), localCoords }, null, 2));
console.log('Wrote newtest-reconstructed-cultivated.json');
