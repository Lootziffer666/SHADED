...
// what changes here is architecture, not the fitting math: fit ONCE, then
// FREEZE (see FREEZE_CAMERA below), never let a local operator re-touch
// it, per the external review's core claim, now confirmed decisive on
// v3c (0.0px VP error, exact f/pp recovery, ~10x better per-cube T once
// frozen).
//
...
// v3d: the real, complete, non-circular end-to-end test. Fit the camera
// ONCE from the noisy edges via the actual closed-form procedure (never
// handed ground truth), then FREEZE it (see FREEZE_CAMERA below) and let
// only the vertex-level operators run.
...
function angleFallbackSeed() {
  ...
  function residualFor(zSign) {
    ...
    let e = 0;
    for (const name of cubeNames) { const verts = state.cubes[name].vertices; for (let i = 0; i < 6; i++) { const fam = famAssignment[name][i], a = verts[i], b = verts[(i + 1) % 6]; if (!vp[fam]) continue; const mid = [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2]; e += angularDist(angleMod180(b[0] - a[0], b[1] - a[1]), angleMod180(vp[fam][0] - mid[0], vp[fam][1] - mid[1])); } }
    return { R, e };
  }
  const cand = [residualFor(1), residualFor(-1)];
  const chosen = cand[0].e <= cand[1].e ? cand[0] : cand[1];
  return { R: chosen.R, pp: pp0, f: f0 };
}
...
console.log('Seed f=' + state2.f.toFixed(1) + ' (truth ' + fTruth.toFixed(1) + ') pp=[' + state2.pp.map(v => v.toFixed(1)) + '] (truth [' + ppTruth.map(v => v.toFixed(1)) + '])  axis pairwise dots after polar-orthonormalize:', [[0, 1], [1, 2], [0, 2]].map(([i, j]) => dot3(state2.R[i], state2.R[j]).toFixed(6)).join(' '));
