...
// Per-cube, per-visible-face seed points (same as the original working
// extraction, before the dilation-drift/vertex-count issues).
const CUBES = {
...
(async () => {
  ...
  const result = await page.evaluate(async ({ dataUrl, CUBES }) => {
    ...
    const usedG = new Array(allEdges.length).fill(false);
    const orderG = allEdges.map((_, i) => i).sort((i, j) => allEdges[j].len - allEdges[i].len);
    const globalClusters = [];
    for (const i of orderG) {
      if (usedG[i]) continue;
      const group = [i]; usedG[i] = true;
      for (const j of orderG) { if (!usedG[j] && angularDist(allEdges[i].angle, allEdges[j].angle) < 15) { group.push(j); usedG[j] = true; } }
      globalClusters.push(group);
    }
    globalClusters.sort((g1, g2) => g2.length - g1.length);
    const top3Clusters = globalClusters.slice(0, 3);
