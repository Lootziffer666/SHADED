...
// New real test image (1672x941), 4 cubes -- much lower contrast than the
// earlier test images (measured: cube faces ~113-130 luminance, but the
// floor IN SHADOW near the cubes is ALSO ~114-127 -- nearly identical.
// Same-posterize-bin flood fill leaked straight through into the floor
// (confirmed: cube2/cube3 masks blew up to ~half the image). Fix: tight
// per-seed luminance TOLERANCE (matches the ~8-unit flat range actually
// measured on a face) instead of a coarse posterize bin, AND a per-cube
// bounding box (from visual inspection) that hard-clips the flood fill --
// a pragmatic safeguard, not a shortcut: it only prevents runaway growth
// far outside where the cube visibly is, it doesn't hand-place vertices.
const CUBES = {
  cube1: { seeds: [[310, 430], [390, 430]], bbox: [260, 320, 435, 545] },
  cube2: { seeds: [[480, 420], [650, 420]], bbox: [410, 245, 730, 595] },
  cube3: { seeds: [[850, 420], [1150, 420]], bbox: [715, 65, 1340, 845] },
  cube4: { seeds: [[1370, 430], [1500, 430]], bbox: [1315, 320, 1555, 555] },
};
...
(async () => {
  ...
  const result = await page.evaluate(async ({ dataUrl, CUBES }) => {
    ...
    function extractCube(seeds, bbox) {
      ...
      for (const [sx, sy] of seeds) {
        ...
        while (stack.length) {
          ...
          for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
            ...
            if (Math.abs(lum(nx, ny) - seedLum) <= TOL) { visited[idx] = 1; stack.push([nx, ny]); }
          }
        }
    ...
    const out = {};
    for (const [name, def] of Object.entries(CUBES)) out[name] = extractCube(def.seeds, def.bbox);
    ...
    const maskColors = { cube1: [255, 0, 0], cube2: [0, 200, 0], cube3: [0, 100, 255], cube4: [255, 150, 0] };
  ...
  fs.writeFileSync('/tmp/claude-0/-home-user-SHADED/28c78061-b0e0-5f7f-bdfd-27d37e45d96b/scratchpad/newtest-raw2d.json', JSON.stringify(dump, null, 2));
  console.log('Wrote newtest-raw2d.json');
  await browser.close();
})();
