...
function runCalibrationPhase(startCand, iters) {
  ...
  return { R: state2.R, pp: state2.pp, f: state2.f, e: globalAngleResidual(), cheiralityBad };
}
...
console.log('\nPhase 1 (calibration-only, ' + candidates.length + '-start, ' + PHASE1_ITERS + ' iters each):', calibrated.map(c => `residual=${c.e.toFixed(2)} cheiralityBad=${(c.cheiralityBad * 100).toFixed(0)}%`).join('  |  '));
// Cheirality (all reconstructed points must sit in FRONT of the camera)
// is a hard, independent constraint the pure angular residual cannot see
// -- rank by it first, exactly as real bundle-adjustment pipelines
// disambiguate otherwise angle-tied camera hypotheses, then break ties by
// residual.
calibrated.sort((a, b) => a.cheiralityBad - b.cheiralityBad || a.e - b.e);
state2.R = calibrated[0].R; state2.pp = calibrated[0].pp; state2.f = calibrated[0].f;
console.log('Phase 1 winner (cheirality-first): f=' + state2.f.toFixed(1) + ' (truth ' + fTruth.toFixed(1) + ') pp=[' + state2.pp.map(v => v.toFixed(1)) + '] (truth [' + ppTruth.map(v => v.toFixed(1)) + '])  cheiralityBad=' + (calibrated[0].cheiralityBad * 100).toFixed(0) + '%');
