...
// originally used -- reads the fixture that scratch-village-extract.mjs
// writes to tools/verify-out/ (run that script first).
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const OUT = path.join(__dirname, 'verify-out');
const raw = JSON.parse(fs.readFileSync(path.join(OUT, 'village-raw2d.json'), 'utf8'));
...
for (let iter = 0; iter < ITERS; iter++) {
  ...
  opSnap(0.15 * decay);
  opRelate(0.1 * decay);
  // opEqualize/opPlace deliberately OFF here -- both assume one isotropic
  // real-world edge length shared by every house, which is false for
  // these anisotropic boxes (footprint != wall height) and is what drove
  // Phase 2 to diverge on the first village attempt. The real, anisotropic
  // shared scale is solved once, jointly, after Phase 2 -- see below.
  opClose();
  opSmooth(0.08 * decay);
...
function solveJointAnisotropic() {
  // Unknowns: [Lx, Ly, Tx_1,Ty_1,Tz_1, Tx_2,Ty_2,Tz_2, ...] (Lz fixed = 1).
  ...
  cubeNames.forEach((name, h) => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const [a, b, c] = lcoords[i];
      const [px, py] = verts[i];
      // Cross-multiplied projection equation f*cam_d = (p_d-pp_d)*cam_2,
      // with cam_d = T_d + a*Lx*R[0][d] + b*Ly*R[1][d] + c*Lz*R[2][d] and
      // Lz fixed at 1 -- moved entirely to one side (RHS=0 for the true
      // unknowns), with the now-constant Lz term moved to the actual RHS.
...
fs.writeFileSync(path.join(OUT, 'village-reconstructed.json'), JSON.stringify({ W, H, R: state2.R, pp: state2.pp, f: state2.f, Lx: joint.Lx, Ly: joint.Ly, Lz: joint.Lz, T: joint.T, vertices: Object.fromEntries(cubeNames.map(n => [n, state.cubes[n].vertices])), localCoords }, null, 2));
console.log('Wrote village-reconstructed.json');
