...
const { W, H, pp: ppTruth, f: fTruth, VP: vpTruth, axes: axesTruth, reconstructed } = svm;
...
console.log('Seed consensus angles (diagnostic only, not used for the seed itself):', seedAngles.map(a => a.toFixed(2) + 'deg'));
...
// v3b: this is the "seed from the already-proven closed-form answer"
// variant. The generic-seed v3 established that conditioning is solved
// (no more crashes, exact structural orthogonality, all params actually
// move) but the operator loop converges to A self-consistent local
// minimum that is NOT the true camera, consistently, regardless of noise
// level or seed chirality -- i.e. a genuine non-convexity/local-minimum
// problem, separate from conditioning. This variant tests a different
// question: starting from the TRUE camera (already verified via
// reprojection error 0.88-4.01px in the original closed-form pipeline),
// do the corrected operators at least PRESERVE correctness while
// correcting the injected noisy vertices, or does the local search still
// wander away from a known-good answer?
const state2 = { R: polarOrthonormalize(axesTruth), pp: [ppTruth[0], ppTruth[1]], f: fTruth };
console.log('Seed = GROUND TRUTH camera. f=' + state2.f.toFixed(1) + ' pp=[' + state2.pp.map(v => v.toFixed(1)) + ']  axis pairwise dots after polar-orthonormalize:', [[0, 1], [1, 2], [0, 2]].map(([i, j]) => dot3(state2.R[i], state2.R[j]).toFixed(6)).join(' '));
...
// v3c: direct test of ChatGPT's sharpest architectural claim on the
// external-review chain ("Der VP wird einmal aus allen Beobachtungen
// bestimmt. Danach wird er nicht von lokalen Agenten herumgeschoben.") --
// v3b already starts opCalibrate from the true camera, yet pp still drifts
// ~200px away from truth over 300 iterations. FREEZE_CAMERA=1 tests
// whether that drift is opCalibrate's doing: skip it entirely, let only
// the vertex-level operators (ALIGN/SNAP/RELATE/EQUALIZE/PLACE/CLOSE/
// SMOOTH) run against a camera model fit once and never touched again.
const FREEZE_CAMERA = process.env.FREEZE_CAMERA === '1';
const ITERS = 300;
const trace = [];
for (let iter = 0; iter < ITERS; iter++) {
  ...
  opAlign(0.15 * decay);
  opSnap(0.15 * decay);
  if (!FREEZE_CAMERA) opCalibrate(0.15 * decay);
  opRelate(0.1 * decay);
