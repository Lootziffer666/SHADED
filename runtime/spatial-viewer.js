...
const representationStatus=document.getElementById('spatial-representation');
...
let program,skyProgram,vao,pointBuffer,count=0,environment,sourceCloud,sourceMetadata=null,basePoints=[],boundaryPoints=[],voxelEditPoints=[],simulation,currentWorldState,grid,path=[],pathIndex=0,mode='orbit',dragging=null,raf=0,last=0,lastWorldUpload=0;
...
function sourceSummary(metadata){
  ...
  if(metadata.format==='SHADED.spatial-point-cloud.v1')return {format:metadata.format,representation:metadata.representation,source:metadata.source,depth:metadata.depth,camera:metadata.camera,scale:metadata.scale,reliability:metadata.reliability,registration:metadata.registration,fusion:metadata.fusion,pointCount:metadata.points?.length||0};
  if(metadata.format==='SHADED.spatial-provider-bundle.v1')return {format:metadata.format,provider:metadata.result?.provider,modelVersion:metadata.result?.modelVersion,metric:metadata.result?.metric,channels:Object.keys(metadata.result?.channels||{})};
  return {format:metadata.format||'UNKNOWN',resolution:metadata.resolution||null};
}
function showRepresentation(text,metadata){if(representationStatus)representationStatus.textContent=text;sourceMetadata=sourceSummary(metadata);log('Darstellung',{text,metadata:sourceMetadata});}
...
openButton?.addEventListener('click',()=>{try{const cloud=window.SHADED?.spatial.pointCloud({step:3});sourceCloud=cloud.points;showRepresentation('VIEW POINTS · STATE VOXELS · RGB + RELATIVE DEPTH · SCALE NORMALIZED · CAMERA ASSUMED · RELIABILITY UNKNOWN · NO REGISTRATION/FUSION',cloud);rebuildEnvironment();reset();setMode('orbit');viewer.hidden=false;last=performance.now();draw(last);}catch(error){window.alert(`Raumansicht nicht verfügbar: ${error.message}`);}});
...
voxelImport?.addEventListener('change',async()=>{const file=voxelImport.files?.[0];if(!file)return;try{const data=JSON.parse(await file.text()),provider=data?.format==='SHADED.spatial-provider-bundle.v1';const world=provider?SparseVoxelWorld.fromProviderBundle(data):SparseVoxelWorld.fromJSON(data);const importedPoints=world.surfacePoints();sourceCloud=importedPoints;environment={points:importedPoints,voxelWorld:world,metrics:{coverage:null,rmse:null,normalizedRmse:null,score:null,status:'NOT_MEASURED'},primitives:[],generated:[]};showRepresentation(provider?`VIEW POINTS · STATE VOXELS · PROVIDER XYZ+RGB · SCALE NORMALIZED · RELIABILITY ${data.result.channels.confidence?'PROVIDER CHANNEL':'UNKNOWN'}`:'VIEW POINTS · STATE VOXELS · IMPORTED VOXEL PROJECT',data);refreshVoxelDerivedGrid();if(fitStatus)fitStatus.textContent=provider?`Provider importiert: ${data.result.provider} · ${world.voxels.size} Oberflächenvoxel · Qualität nicht gemessen`:`Voxel-Projekt importiert · ${world.voxels.size} Oberflächenvoxel · Qualität nicht gemessen`;log(provider?'Provider-Bundle importiert':'Voxel-Projekt importiert',{provider:provider?data.result.provider:undefined,model:provider?data.result.modelVersion:undefined,voxels:world.voxels.size,free:world.free.size,quality:'NOT_MEASURED'});}catch(error){log('Räumlicher Import fehlgeschlagen',error.message);}});
...
if(window.SHADED?.spatial)window.SHADED.spatial.voxel={
  ...
  importProviderBundle:bundle=>{const world=SparseVoxelWorld.fromProviderBundle(bundle),points=world.surfacePoints();sourceCloud=points;environment={points,voxelWorld:world,metrics:{coverage:null,rmse:null,normalizedRmse:null,score:null,status:'NOT_MEASURED'},primitives:[],generated:[]};showRepresentation(`VIEW POINTS · STATE VOXELS · PROVIDER XYZ+RGB · SCALE NORMALIZED · RELIABILITY ${bundle.result.channels.confidence?'PROVIDER CHANNEL':'UNKNOWN'}`,bundle);refreshVoxelDerivedGrid();return {provider:bundle.result.provider,modelVersion:bundle.result.modelVersion,representation:{visible:'POINTS',state:'VOXELS',meshRendered:false},quality:'NOT_MEASURED',voxels:world.voxels.size,free:world.free.size};},
