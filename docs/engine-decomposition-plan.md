...
| 6 | Spieler/Feuer (Runde 4): `spawnPlayer`, `dash`, `playerTick`, `igniteFire`, `fireTick`, `drawPlayer` | 2242–2890 (alt) | A, groß | ✅ **Extrahiert (Stufe 4)** → `runtime/player-fire.mjs`. `fireUniforms()` bleibt bewusst in `shaded-engine.mjs`, weil es direkt `gl.uniform4fv`/`gl.uniform1f` schreibt (Kopplung an den WebGL-Kern, Cluster 2) — liest den geteilten `fires`-Zustand nur über die Bridge. Siehe Stufe-4-Eintrag unten für die Kopplungsbefunde. |
...
| 8 | UI/Akt-Bindings (`syncSliders`, `applyAct`, `tickActBlend`, `setShowcaseCaption`, `stopShowcase`, `waitForReady`, `tickShowcase`) | 3430–3562 | B | `editor/` (inkl. `app.js`) ist inzwischen vollständig entfernt (architecture/ui-zero-contracts, `docs/UI_ZERO.md`); die verbleibende Logik hier IST die kanonische Implementierung, nicht dupliziert. Per UI_ZERO_REMNANT_AUDIT.md Abschnitt 2 bereits gegen fehlende DOM-Elemente abgesichert (Null-Guards statt harter `getElementById`-Zugriffe) — bleibt hier, keine eigene Extraktion nötig, solange kein UI-Client sie eigenständig braucht. |
...
- **Stufe 3 — Weather-/Ökosystem-Partikel** → `runtime/weather-particles.mjs`. Größte Stufe
  ...
  siehe Zeile-6b-Eintrag oben. Danach, außerhalb dieser Extraktionsreihe: auf gemeldeten
  Produktfehler hin (Regen/Schnee/Hagel verhielten sich wie ein flaches Fullscreen-Overlay,
  fielen nie sichtbar AUF Dächer/Boden) wurde `analyze()`s bereits vorhandene K7-Himmel-
  Flutfüllung als `skyGrid`/`window.SHADED.skyAt(u,v)`/`hasSkyRegion()` persistiert und alle
  drei Partikelsysteme landen jetzt unconditional auf `!skyAt(u,v)` statt auf einem festen
  Bildschirm-`v`-Schwellwert — wichtig für den isometrischen Kompositionsstil des Spiels, in
  dem Boden/Dächer über das ganze Bild verteilt sichtbar sind, nicht nur im unteren Rand.
- **Stufe 4 — Spieler/Feuer** → `runtime/player-fire.mjs`. Neuer Kopplungsfund gegenüber
  Stufe 3: `spawnPlayer()` scannte bisher direkt `classGrid`/`AW`/`AH` nach dem nächsten
  Pfad-Pixel — der erste direkte Zugriff eines zu extrahierenden Clusters auf die
  Material-Wahrheit selbst (nicht nur auf `getMaterialTypeAt()`). Gelöst durch eine neue,
  in `shaded-engine.mjs` verbleibende Hilfsfunktion `findSpawnPoint()` (gleich neben
  `getMaterialTypeAt()`), die die Suche kapselt und nur `{u,v}` über
  `window.SHADED_ENGINE_INTERNAL.findSpawnPoint` zurückgibt — im Geiste von Invariante 2
  bleibt `classGrid` unexportiert, das Modul bekommt nur ein Verhalten, keine Rohdaten.
  `fireUniforms()` bleibt bewusst im Kern (schreibt GL-Uniforms direkt) und liest den
  jetzt im Modul lebenden `fires`-Array-Zustand über die Bridge. `window.SHADED.player`/
  `.fire` werden jetzt von `runtime/player-fire.mjs` selbst angehängt statt im großen
  `window.SHADED`-Objektliteral zu stehen; die verbleibenden Aufrufer im Kern
  (`erstellen()`, `elementPreset()`, der Showcase-Trigger, `tickWorld()`, `setTime()`,
  `worldState()`) wurden auf die öffentliche API bzw. die Bridge umgestellt, je nachdem,
  ob der Aufruf von außerhalb des Moduls (öffentlich) oder als reiner geteilter Zustand
  (Bridge) Sinn ergab. `npm run check` grün, `tools/verify.js` grün (Klassenregression,
