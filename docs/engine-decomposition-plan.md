...
| 5 | Storyboard + Blitze (`defaultStoryboard…showcaseStoryboard`, `renderStory`, `playStory`, `stopStory`, `tickStory`, `tickLightning`) | 2084–2241 | A | **Nächster Kandidat (Stufe 2).** Bereits von CLAUDE.md als künftiges `StoryboardTimeline`-Fassaden-Ziel benannt ("erst migrieren, wenn der Ersatz nachweislich mindestens gleichwertig ist" gilt für die FASSADE, nicht für das reine Verschieben der Engine-internen Logik in ein eigenes Modul). Kopplung: `CUR`/`PARAMS`/`ACTS`, `#story-list` DOM, Akt-Blend-Zustand. |
...
| 7 | SWIFT-Actor-Bridge (`actorDepthBrightness`, `parseActorManifest`, `drawActors`, `addActor`, `loadActorPair`) + `ensureElementScene`/`elementPreset` | 2891–3429 | A | Guter Kandidat für eine frühe Stufe (Stufe 3): bereits gut isoliert, hat eigenen Regressionstest `tools/verify-actors.js`. Kopplung: `fog`/`dayNight` für `globalAlpha`, Overlay-Canvas, Tiefen-Textur (Unit 6). |
