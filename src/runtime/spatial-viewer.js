...
openButton?.addEventListener('click',()=>{try{sourceCloud=window.SHADED?.spatial.pointCloud({step:3}).points;rebuildEnvironment();reset();setMode('orbit');viewer.hidden=false;last=performance.now();draw(last);}catch(error){window.alert(`Raumansicht nicht verfügbar: ${error.message}`);}});
